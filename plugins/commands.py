...
from database.users_chats_db import mongo_db, sqlite_db  # Ensure correct database imports

MAX_WARNINGS = 3  # Change this based on your requirement

@Client.on_message(filters.command("warn") & filters.group)
async def warn_user(client: Client, message: Message):
    """Warn a user. If max warnings are reached, kick them."""
    ...
    warnings = await sqlite_db.get_warnings(chat_id, user_id)
    warnings += 1

    # Update warnings in the database
    await sqlite_db.add_warning(chat_id, user_id)

    if warnings >= MAX_WARNINGS:
        await message.reply(f"{message.reply_to_message.from_user.mention} has reached the max warnings and will be kicked.")
        await client.ban_chat_member(chat_id, user_id)  # Kick the user
        await sqlite_db.reset_warnings(chat_id, user_id)  # Reset warnings after kick
    else:
        await message.reply(f"{message.reply_to_message.from_user.mention} has been warned. Warnings: {warnings}/{MAX_WARNINGS}")
...
async def kick_user(client: Client, message: Message):
    ...
    await message.reply(f"{message.reply_to_message.from_user.mention} has been kicked from the group!")
